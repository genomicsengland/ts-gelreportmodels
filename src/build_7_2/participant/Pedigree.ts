...
export class Pedigree extends JSONHelper {
  ...
  calculateRelationToProband() {
    ...
    const pairs = combination(this.members, 2).filter(pair =>
      pair[0].isProband || pair[1].isProband ? true : false
    );
    pairs.forEach(pair => {
      let [m1, m2] = pair;
  ...
  isFirstCousing = (m1: PedigreeMember, m2: PedigreeMember) => {
    const m1Mother = m1.motherId
      ? this.getPedigreeById(m1.motherId)
      : undefined;
    const m1Father = m1.fatherId
      ? this.getPedigreeById(m1.fatherId)
      : undefined;
    const maternalRelation = m1Mother
      ? this.isUncleOrAunt(m1Mother, m2)
      : undefined;
    const paternalRelation = m1Father
      ? this.isUncleOrAunt(m1Father, m2)
      : undefined;
  ...
  isGrandParent = (m1: PedigreeMember, m2: PedigreeMember) => {
    const m2Mother = m2.motherId
      ? this.getPedigreeById(m2.motherId)
      : undefined;
    const m2Father = m2.fatherId
      ? this.getPedigreeById(m2.fatherId)
      : undefined;
  ...
  getAncestors = (member: PedigreeMember) => {
    ...
    while (!q.isEmpty()) {
      let ancestor = q.pop();
      if (ancestor) {
        let mother = ancestor.motherId
          ? this.getPedigreeById(ancestor.motherId)
          : undefined;
        if (mother) {
          q.push(mother);
          ancestors.add(mother);
        }
        let father = ancestor.fatherId
          ? this.getPedigreeById(ancestor.fatherId)
          : undefined;
        if (father) {
          q.push(father);
