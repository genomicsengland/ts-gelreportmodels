...
var Pedigree = /** @class */ (function (_super) {
    ...
    function Pedigree() {
        ...
        _this.isChild = function (m1, m2) {
            // get pedigree ID
            var parentPedigreeId = m2.sex === Sex_1.Sex.MALE ?
                m1.fatherId : m2.sex === Sex_1.Sex.FEMALE ?
            ...
            if (parentPedigreeId && m2.pedigreeId && m2.pedigreeId === parentPedigreeId) {
                switch (m1.sex) {
                    ...
                    default:
                        return FamiliarRelationship_1.FamiliarRelationship.ChildOfUnknownSex;
                }
            }
        };
        /**
            ...
            if (m2.motherId) {
                ...
                if (m2Mother && _this.isSibling(m1, m2Mother) && m2Mother !== m1) {
                    switch (m1.sex) {
                        ...
                        default:
                            return FamiliarRelationship_1.FamiliarRelationship.MaternalUncleOrAunt;
                    }
                }
            }
            if (m2.fatherId) {
                ...
                if (m2Father && _this.isSibling(m1, m2Father) && m2Father !== m1) {
                    switch (m1.sex) {
                        ...
                        default:
                            return FamiliarRelationship_1.FamiliarRelationship.PaternalUncleOrAunt;
                    }
                }
            }
        };
        _this.isFirstCousing = function (m1, m2) {
            var m1Mother = m1.motherId ? _this.getPedigreeById(m1.motherId) : undefined;
            var m1Father = m1.fatherId ? _this.getPedigreeById(m1.fatherId) : undefined;
            var maternalRelation = m1Mother ? _this.isUncleOrAunt(m1Mother, m2) : undefined;
            var paternalRelation = m1Father ? _this.isUncleOrAunt(m1Father, m2) : undefined;
            ...
            if (m1Mother && maternalRelation) {
                switch (maternalRelation) {
                    case FamiliarRelationship_1.FamiliarRelationship.MaternalAunt:
                        return FamiliarRelationship_1.FamiliarRelationship.MaternalCousinSister;
                    case FamiliarRelationship_1.FamiliarRelationship.PaternalAunt:
                        return FamiliarRelationship_1.FamiliarRelationship.PaternalCousinSister;
                    default:
            ...
            if (m1Father && paternalRelation) {
                switch (paternalRelation) {
                    case FamiliarRelationship_1.FamiliarRelationship.PaternalUncle:
                        return FamiliarRelationship_1.FamiliarRelationship.PaternalCousinBrother;
                    case FamiliarRelationship_1.FamiliarRelationship.MaternalUncle:
                        return FamiliarRelationship_1.FamiliarRelationship.MaternalCousinBrother;
                    default:
                        return FamiliarRelationship_1.FamiliarRelationship.Cousin;
                }
            }
        ...
        _this.isGrandParent = function (m1, m2) {
            var m2Mother = m2.motherId ? _this.getPedigreeById(m2.motherId) : undefined;
            var m2Father = m2.fatherId ? _this.getPedigreeById(m2.fatherId) : undefined;
        ...
        _this.isSpouse = function (m1, m2) {
            ...
            if (lodash_1.intersectionBy(Array.from(m1Offsprings), Array.from(m2Offsprings), 'pedigreeId').length > 0) {
        ...
        _this.isUnrelated = function (m1, m2) {
            ...
            if (!lodash_1.intersectionBy(m1Ancestors, m2Ancestors, 'pedigreeId').length) {
        ...
        _this.getAncestors = function (member) {
            ...
            while (!q.isEmpty()) {
                var ancestor = q.pop();
                if (ancestor) {
                    var mother = ancestor.motherId ? _this.getPedigreeById(ancestor.motherId) : undefined;
                    if (mother) {
                        q.push(mother);
                        ancestors.add(mother);
                    }
                    var father = ancestor.fatherId ? _this.getPedigreeById(ancestor.fatherId) : undefined;
                    if (father) {
                        q.push(father);
    ...
    Pedigree.prototype.calculateRelationToProband = function () {
        var _this = this;
        var proband = this.members.filter(function (member) { return member.isProband; })[0];
        if (!proband) {
            return;
        }
        // get all combinations, but remove any pair where one of the member isn't proband
        var pairs = js_combinatorics_1.combination(this.members, 2).filter(function (pair) { return pair[0].isProband || pair[1].isProband ? true : false; });
